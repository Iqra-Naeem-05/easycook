...
class RegisterSerializer(serializers.ModelSerializer):
    ...
    confirm_password = serializers.CharField(write_only=True)  # Not in the model, used only for validation
...
class ChefProfileSerializer(serializers.ModelSerializer):
    ...
    def update(self, instance, validated_data):
        """
        Override update() to delete old image properly when a new one is uploaded.
        """
...
class DishSerializer(serializers.ModelSerializer):
    ...
    def validate_picture(self, picture):
        print("is it picture validation:", picture)
        try:
    ...
    def validate_serving_number(self, value):
        """Ensure serving number is a positive integer"""
        if value <= 0:
    ...
    def update(self, instance, validated_data):
        ...
        if new_picture and instance.picture and instance.picture.name != "defaults/default_dish.png":
            try:
                ...
                # Handle the exception (e.g., log the error, notify administrators)
                pass
        return super().update(instance, validated_data)
...
class BookingSerializer(serializers.ModelSerializer):
    ...
    #     if value < tomorrow or value > seven_days_later:
    #         raise serializers.ValidationError(
    #             f"Date must be between {tomorrow} and {seven_days_later}"
    #         )
    #     return value
    ...
    def validate(self, data):
        ...
        date_ = data['date']
        slot = data['slot']  # 'breakfast', 'lunch', or 'dinner'
        ...
        # booking_type = data['booking_type']  # 'urgent' or 'prebooking'
        ...
        print('existing_bookingggg', existing_booking)
        if self.instance:
