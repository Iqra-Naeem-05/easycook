...
def register_user(request):
    ...
    if serializer.is_valid():
        ...
        # Ensure session ID is set in cookies
        response = Response({"message": "User registered and logged in successfully!"}, status=status.HTTP_201_CREATED)
        # However, request.session.session_key might be None right after login.
        # Solution: Ensure a session is created before setting the cookie:
        request.session.create()
        response.set_cookie("sessionid", request.session.session_key, httponly=True)   
        return response 

    # return Response({"message": "User registered not done!"}, serializer.errors, status=status.HTTP_400_BAD_REQUEST)
...
def login_user(request):
    ...
    if serializer.is_valid():
        ...
        login(request, user)  # Logs in the user (session-based)
...
#     if request.method == 'DELETE':  # Fetch profile
#         # serializer = ChefProfileSerializer(profile)
...
def chef_availability(request):
    ...
    if request.method == 'PATCH':
        ...
        if serializer.is_valid():
            ...
            print("Updated fields:", serializer.validated_data) 
...
def featured_chefs(request):
    ...
    # Exclude the requesting user if they are a chef
    if request.user.is_authenticated and request.user.role == "chef":
        chefs = chefs.exclude(id=request.user.id)
...
def customer_bookings(request):
    bookings = Booking.objects.filter(customer=request.user).annotate(
        custom_priority=Case(
            ...
            # When(status='pending', booking_type='urgent', slot='breakfast', then=Value(1)),
            # When(status='pending', booking_type='urgent', slot='lunch', then=Value(2)),
            # When(status='pending', booking_type='urgent', slot='dinner', then=Value(3)),
            # When(status='pending', booking_type='prebooking', slot='breakfast', then=Value(4)),
            # When(status='pending', booking_type='prebooking', slot='lunch', then=Value(5)),
            # When(status='pending', booking_type='prebooking', slot='dinner', then=Value(6)),
            default=Value(13),
    ...
    # ).order_by('custom_priority', 'status_order', '-date', '-created_at')
...
def chef_upcoming_bookings(request):
    bookings = Booking.objects.filter(chef=request.user).annotate(
        ...
        # ,
        # booking_type_order = Case(
        #     When(booking_type='urgent', then=Value(1)),
        #     When(booking_type='prebooking', then=Value(2)),
        #     default=Value(3),
        #     output_field=IntegerField(),
        # ),
        # status_order=Case(
        ...
        #     When(status='expired', then=Value(6)),
        #     default=Value(7),
        #     output_field=IntegerField(),
        # )
    ).order_by('custom_priority')
    # ).order_by('custom_priority', 'booking_type_order', 'status_order', '-date')
...
def update_booking_status(request, booking_id):
    try:
        ...
        if new_status == 'confirmed':
            # Prevent confirming if another booking is already confirmed for same slot/date/type
...
def check_and_expire_booking(booking):
    ...
    if booking.booking_type == 'urgent':
        ...
        return  # No action needed for other types
...
def check_and_complete_booking(booking):
    ...
    slot_end_str = time_range[1]  # e.g., '10:00'
...
def rate_chef(request, chef_id):
    print(f"Request user: {request.user} ({type(request.user)})")

    try:
        # Fetch the chef object
...
def get_chef_rating(request, chef_id):
    try:
        ...
        print(f"Error in get_chef_rating: {e}")  # Optional: Useful during debugging
