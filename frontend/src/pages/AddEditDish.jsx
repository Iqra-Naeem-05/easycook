...
const AddEditDish = () => { 
  const { user  } = useContext(AuthContext); // Access global auth state
  const [errors, setErrors] = useState("");
  const navigate = useNavigate();
  const { dishId } = useParams(); // Get dishId from URL parameters
  ...
  useEffect(() => {
    if (dishId) {
      // Fetch existing dish details for editing
      axios
        .get(`/get-dish/${dishId}/`)
        .then((response) => {
          ...
          setFormData({
            ...
            price: data.price ?? "",
            picture: data.picture || null, // Keep existing picture URL
          });
        })
        .catch((err) => {
  ...
  const handleSubmit = async (e) => {
    ...
    //   data.append("picture", formData.picture); // Only append if a new file is selected
    // }
    if (selectedFile) {
  ...
  try {
    ...
    navigate(`/chef-dishes/${user.id}`); // Redirect after successful submission
    // navigate("/"); // Redirect after successful submission
  } catch (err) {
    // setError("Error saving dish:", error.response.data);
...
>
  {/* <i className="fas fa-camera" style={{ fontSize: '40px', color: '#6c757d' }}></i> */}
